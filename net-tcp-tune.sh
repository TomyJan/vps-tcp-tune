...
# ============================================================================
# 御三家模型列表查看器 (Claude / GPT / Gemini)
# ============================================================================

MODEL_VIEWER_INSTALL_DIR="/opt/model-list-viewer"
MODEL_VIEWER_SCRIPT="${MODEL_VIEWER_INSTALL_DIR}/server.mjs"
MODEL_VIEWER_CONFIG="${MODEL_VIEWER_INSTALL_DIR}/config.json"
MODEL_VIEWER_SERVICE="model-list-viewer"
MODEL_VIEWER_DEFAULT_PORT="18800"

# 检测模型列表查看器状态
model_viewer_check_status() {
    if [ ! -f "$MODEL_VIEWER_SCRIPT" ]; then
        echo "not_installed"
    elif systemctl is-active "$MODEL_VIEWER_SERVICE" &>/dev/null; then
        echo "running"
    else
        echo "stopped"
    fi
...
# 获取端口
model_viewer_get_port() {
    if [ -f "$MODEL_VIEWER_CONFIG" ]; then
        local port
        port=$(grep -o '"port"[[:space:]]*:[[:space:]]*[0-9]*' "$MODEL_VIEWER_CONFIG" | grep -o '[0-9]*$')
        echo "${port:-$MODEL_VIEWER_DEFAULT_PORT}"
    else
        echo "$MODEL_VIEWER_DEFAULT_PORT"
    fi
}
...
model_viewer_deploy() {
    clear
    echo -e "${gl_kjlan}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${gl_bai}"
    echo -e "${gl_kjlan}  部署御三家模型列表查看器${gl_bai}"
    echo -e "${gl_kjlan}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${gl_bai}"
    echo ""
    echo "实时查询 Claude / GPT / Gemini 官方模型列表"
    echo "通过各家官方 API 获取最新模型信息"
    echo ""
    ...
    # 配置
    echo ""
    read -e -p "监听端口 [${MODEL_VIEWER_DEFAULT_PORT}]: " mv_port
...
CONFIGEOF
    ...
    # 写入服务脚本
    cat > "$MODEL_VIEWER_SCRIPT" << 'SERVEREOF'
import http from 'node:http';
import https from 'node:https';
import fs from 'node:fs';

const CFG_PATH = '/opt/model-list-viewer/config.json';
let cfg;
try { cfg = JSON.parse(fs.readFileSync(CFG_PATH, 'utf-8')); }
catch (e) { console.error('Config error:', e.message); process.exit(1); }

const PORT = cfg.port || 18800;
const TTL = (cfg.cache_ttl_seconds || 300) * 1000;
const cache = {};
...
async function fetchClaude() {
    ...
    try {
        ...
        return { provider: 'claude', error: e.message, models: [] };
    }
}

async function fetchOpenAI() {
    const c = getCached('openai');
    if (c) return c;
    ...
    try {
        ...
        return { provider: 'openai', error: e.message, models: [] };
    }
}

async function fetchGemini() {
    const c = getCached('gemini');
    if (c) return c;
    ...
    try {
        ...
        return { provider: 'gemini', error: e.message, models: [] };
    }
}
...
body{font-family:-apple-system,"PingFang SC","Microsoft YaHei",sans-serif;background:#f3f0ff;color:#333;min-height:100vh}
.banner{background:linear-gradient(135deg,#7c3aed 0%,#a855f7 40%,#c084fc 70%,#e9d5ff 100%);padding:48px 0 80px;text-align:center}
.banner h1{color:#fff;font-size:28px;font-weight:700;letter-spacing:1px}
.banner .sub{color:rgba(255,255,255,.8);font-size:14px;margin-top:8px}
.container{max-width:1000px;margin:-48px auto 40px;padding:0 20px}
.toolbar{display:flex;justify-content:flex-end;margin-bottom:16px;gap:12px;align-items:center}
.toolbar .time{font-size:12px;color:#999}
.search-input{width:220px;padding:8px 14px;border:1px solid #e9d5ff;border-radius:8px;font-size:13px;outline:none;transition:border .2s}
.search-input:focus{border-color:#7c3aed}
.refresh-btn{background:#7c3aed;color:#fff;border:none;padding:8px 20px;border-radius:8px;cursor:pointer;font-size:13px;transition:all .2s}
.refresh-btn:hover{background:#6d28d9}
.refresh-btn:disabled{opacity:.6;cursor:not-allowed}
.stats-row{display:flex;gap:16px;margin-bottom:24px}
.stat-card{flex:1;background:#fff;border-radius:16px;padding:20px 24px;box-shadow:0 2px 12px rgba(124,58,237,.08);text-align:center}
.stat-card .num{font-size:32px;font-weight:700}
.stat-card .label{font-size:13px;color:#888;margin-top:4px}
.c-purple .num{color:#7c3aed}
.c-green .num{color:#059669}
.c-blue .num{color:#2563eb}
.c-red .num{color:#e11d48}
.provider-card{background:#fff;border-radius:16px;box-shadow:0 2px 12px rgba(124,58,237,.08);margin-bottom:24px;overflow:hidden}
.provider-header{padding:20px 28px;display:flex;align-items:center;gap:12px;border-bottom:1px solid #f0ecff}
.provider-icon{width:40px;height:40px;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:20px;color:#fff;flex-shrink:0;font-weight:700}
.icon-claude{background:linear-gradient(135deg,#7c3aed,#a855f7)}
.icon-gpt{background:linear-gradient(135deg,#059669,#34d399)}
.icon-gemini{background:linear-gradient(135deg,#2563eb,#60a5fa)}
.provider-name{font-size:17px;font-weight:600}
.provider-badge{font-size:11px;background:#f3f0ff;color:#7c3aed;padding:3px 10px;border-radius:20px;font-weight:500;margin-left:auto}
.provider-card table{width:100%;border-collapse:collapse}
.provider-card th{text-align:left;padding:12px 28px;font-size:12px;color:#999;font-weight:500;text-transform:uppercase;letter-spacing:.5px;background:#faf9ff;border-bottom:1px solid #f0ecff}
.provider-card td{padding:14px 28px;font-size:14px;border-bottom:1px solid #f8f6ff}
.provider-card tr:last-child td{border-bottom:none}
.provider-card tbody tr:hover td{background:#faf8ff}
.model-id{font-family:"SF Mono","Menlo","Consolas",monospace;font-size:13px;color:#7c3aed;background:#f5f0ff;padding:3px 8px;border-radius:6px;display:inline-block}
.num-col{color:#bbb;font-size:13px;width:36px}
.desc{color:#666;max-width:300px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
.note{padding:14px 28px;background:#faf9ff;border-top:1px solid #f0ecff;font-size:13px;color:#888}
.error-msg{padding:20px 28px;color:#e11d48;font-size:14px}
.loading{text-align:center;padding:40px;color:#999}
.spinner{display:inline-block;width:20px;height:20px;border:2px solid #e9d5ff;border-top-color:#7c3aed;border-radius:50%;animation:spin .8s linear infinite;margin-right:8px;vertical-align:middle}
@keyframes spin{to{transform:rotate(360deg)}}
.footer{text-align:center;padding:20px 0 40px;color:#bbb;font-size:12px}
@media(max-width:640px){.stats-row{flex-wrap:wrap}.stat-card{min-width:calc(50% - 8px)}.provider-card th,.provider-card td{padding:10px 16px}.banner{padding:32px 0 64px}.banner h1{font-size:22px}}
</style>
</head>
<body>
<div class="banner">
<h1>御三家模型列表</h1>
<div class="sub">实时查询 &middot; Claude / GPT / Gemini 官方 API</div>
</div>
<div class="container">
<div class="toolbar">
<input type="text" class="search-input" id="search" placeholder="搜索模型 ID..." oninput="filterModels()">
<span class="time" id="update-time"></span>
<button class="refresh-btn" id="refresh-btn" onclick="loadAll()">刷新</button>
</div>
<div class="stats-row">
<div class="stat-card c-purple"><div class="num" id="cnt-claude">-</div><div class="label">Claude</div></div>
<div class="stat-card c-green"><div class="num" id="cnt-openai">-</div><div class="label">GPT</div></div>
<div class="stat-card c-blue"><div class="num" id="cnt-gemini">-</div><div class="label">Gemini</div></div>
<div class="stat-card c-red"><div class="num" id="cnt-total">-</div><div class="label">Total</div></div>
</div>
<div id="providers"><div class="loading"><span class="spinner"></span>正在查询模型列表...</div></div>
</div>
...
try{var r=await fetch('/api/models/all?refresh=1');var d=await r.json();render(d);}
catch(e){document.getElementById('providers').innerHTML='<div class="error-msg">查询失败: '+e.message+'</div>';}
finally{btn.disabled=false;btn.textContent='刷新';}
}
function render(d){
var cc=d.claude&&d.claude.models?d.claude.models.length:0;
var oc=d.openai&&d.openai.models?d.openai.models.length:0;
var gc=d.gemini&&d.gemini.models?d.gemini.models.length:0;
document.getElementById('cnt-claude').textContent=cc;
document.getElementById('cnt-openai').textContent=oc;
document.getElementById('cnt-gemini').textContent=gc;
document.getElementById('cnt-total').textContent=cc+oc+gc;
document.getElementById('update-time').textContent='更新: '+new Date().toLocaleString('zh-CN');
document.getElementById('footer').textContent='共 '+(cc+oc+gc)+' 个模型 | Claude '+cc+' + GPT '+oc+' + Gemini '+gc;
var h='';
h+=renderProvider('claude','Claude (Anthropic)','C','icon-claude',d.claude);
h+=renderProvider('openai','GPT (OpenAI)','G','icon-gpt',d.openai);
h+=renderProvider('gemini','Gemini (Google)','G','icon-gemini',d.gemini);
document.getElementById('providers').innerHTML=h;
}
function renderProvider(key,name,icon,iconCls,pd){
var cnt=pd&&pd.models?pd.models.length:0;
var h='<div class="provider-card"><div class="provider-header">';
h+='<div class="provider-icon '+iconCls+'">'+icon+'</div>';
h+='<div class="provider-name">'+name+'</div>';
h+='<div class="provider-badge">'+cnt+' 个模型</div></div>';
...
h+='</tr>';}
h+='</tbody></table>';
}else{h+='<div class="note">暂无模型数据</div>';}
if(pd&&pd.fetched_at){h+='<div class="note">缓存时间: '+new Date(pd.fetched_at).toLocaleString('zh-CN')+'</div>';}
h+='</div>';return h;
}
function filterModels(){
var q=document.getElementById('search').value.toLowerCase();
var rows=document.querySelectorAll('.provider-card tbody tr');
for(var i=0;i<rows.length;i++){var id=rows[i].querySelector('.model-id');
if(id){rows[i].style.display=id.textContent.toLowerCase().indexOf(q)>=0?'':'none';}}
}
...
const server = http.createServer(async (req, res) => {
    ...
    if (url === '/api/models/all') {
        if (params.get('refresh') === '1') { delete cache.claude; delete cache.openai; delete cache.gemini; }
        const [claude, openai, gemini] = await Promise.allSettled([fetchClaude(), fetchOpenAI(), fetchGemini()]);
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({
            claude: claude.status === 'fulfilled' ? claude.value : { error: claude.reason?.message, models: [] },
            openai: openai.status === 'fulfilled' ? openai.value : { error: openai.reason?.message, models: [] },
            gemini: gemini.status === 'fulfilled' ? gemini.value : { error: gemini.reason?.message, models: [] }
        }));
        return;
    }

    if (url === '/api/models/claude') {
        if (params.get('refresh') === '1') delete cache.claude;
    ...
    if (url === '/api/models/openai') {
        if (params.get('refresh') === '1') delete cache.openai;
    ...
    if (url === '/api/models/gemini') {
        if (params.get('refresh') === '1') delete cache.gemini;
...
server.listen(PORT, () => {
    console.log('[Model List Viewer] http://0.0.0.0:' + PORT);
    console.log('Keys: Claude=' + (cfg.anthropic_api_key ? 'Yes' : 'No')
        + ' OpenAI=' + (cfg.openai_api_key ? 'Yes' : 'No')
        + ' Gemini=' + (cfg.google_api_key ? 'Yes' : 'No'));
});
SERVEREOF
    ...
    cat > "/etc/systemd/system/${MODEL_VIEWER_SERVICE}.service" << SVCEOF
[Unit]
Description=Model List Viewer (Claude/GPT/Gemini)
After=network.target

[Service]
Type=simple
ExecStart=$(which node) ${MODEL_VIEWER_SCRIPT}
Restart=on-failure
RestartSec=5
WorkingDirectory=${MODEL_VIEWER_INSTALL_DIR}
...
SVCEOF
    ...
    systemctl daemon-reload
    systemctl enable "$MODEL_VIEWER_SERVICE" 2>/dev/null
    systemctl start "$MODEL_VIEWER_SERVICE"
    sleep 2

    if systemctl is-active "$MODEL_VIEWER_SERVICE" &>/dev/null; then
        local server_ip
        server_ip=$(curl -s4 --max-time 3 ifconfig.me 2>/dev/null || curl -s4 --max-time 3 ip.sb 2>/dev/null || echo "你的IP")
        echo ""
        echo -e "${gl_lv}✅ 御三家模型列表部署成功！${gl_bai}"
        echo ""
        echo -e "访问地址: ${gl_huang}http://${server_ip}:${mv_port}${gl_bai}"
        echo ""
        echo "API 接口:"
        echo -e "  全部模型: ${gl_zi}http://${server_ip}:${mv_port}/api/models/all${gl_bai}"
        echo -e "  Claude:   ${gl_zi}http://${server_ip}:${mv_port}/api/models/claude${gl_bai}"
        echo -e "  OpenAI:   ${gl_zi}http://${server_ip}:${mv_port}/api/models/openai${gl_bai}"
        echo -e "  Gemini:   ${gl_zi}http://${server_ip}:${mv_port}/api/models/gemini${gl_bai}"
    else
        echo -e "${gl_hong}❌ 服务启动失败，请检查日志：journalctl -u ${MODEL_VIEWER_SERVICE} -n 20${gl_bai}"
    fi
...
model_viewer_config() {
    clear
    echo -e "${gl_kjlan}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${gl_bai}"
    echo -e "${gl_kjlan}  修改模型列表查看器配置${gl_bai}"
    echo -e "${gl_kjlan}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${gl_bai}"
    echo ""
...
CONFIGEOF
    ...
    case "$confirm" in
        [Yy])
            systemctl restart "$MODEL_VIEWER_SERVICE" 2>/dev/null
            sleep 2
            if systemctl is-active "$MODEL_VIEWER_SERVICE" &>/dev/null; then
                echo -e "${gl_lv}✅ 服务已重启${gl_bai}"
            else
...
model_viewer_status() {
    clear
    echo -e "${gl_kjlan}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${gl_bai}"
    echo -e "${gl_kjlan}  模型列表查看器状态${gl_bai}"
    echo -e "${gl_kjlan}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${gl_bai}"
    echo ""
    systemctl status "$MODEL_VIEWER_SERVICE" --no-pager 2>/dev/null || echo "服务未安装"
    echo ""
    if [ -f "$MODEL_VIEWER_CONFIG" ]; then
        local cur_port server_ip
        cur_port=$(model_viewer_get_port)
        server_ip=$(curl -s4 --max-time 3 ifconfig.me 2>/dev/null || curl -s4 --max-time 3 ip.sb 2>/dev/null || echo "你的IP")
        echo -e "${gl_kjlan}访问地址:${gl_bai}"
        echo -e "  ${gl_huang}http://${server_ip}:${cur_port}${gl_bai}"
    fi
    break_end
}
...
model_viewer_logs() {
    ...
    echo -e "${gl_huang}按 Ctrl+C 退出日志${gl_bai}"
    echo ""
    journalctl -u "$MODEL_VIEWER_SERVICE" -f --no-pager
}

# 启动模型列表查看器
model_viewer_start() {
    systemctl start "$MODEL_VIEWER_SERVICE" 2>/dev/null
    sleep 1
    if systemctl is-active "$MODEL_VIEWER_SERVICE" &>/dev/null; then
        echo -e "${gl_lv}✅ 服务已启动${gl_bai}"
    else
...
# 停止模型列表查看器
model_viewer_stop() {
    systemctl stop "$MODEL_VIEWER_SERVICE" 2>/dev/null
    echo -e "${gl_lv}✅ 服务已停止${gl_bai}"
    break_end
}

# 重启模型列表查看器
model_viewer_restart() {
    systemctl restart "$MODEL_VIEWER_SERVICE" 2>/dev/null
    sleep 1
    if systemctl is-active "$MODEL_VIEWER_SERVICE" &>/dev/null; then
        echo -e "${gl_lv}✅ 服务已重启${gl_bai}"
...
# 卸载模型列表查看器
model_viewer_uninstall() {
    clear
    echo -e "${gl_kjlan}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${gl_bai}"
    echo -e "${gl_hong}  卸载模型列表查看器${gl_bai}"
    echo -e "${gl_kjlan}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${gl_bai}"
    echo ""
    echo -e "${gl_huang}警告: 此操作将删除模型列表查看器及其所有配置！${gl_bai}"
    echo ""
    ...
    echo "正在停止服务..."
    systemctl stop "$MODEL_VIEWER_SERVICE" 2>/dev/null
    systemctl disable "$MODEL_VIEWER_SERVICE" 2>/dev/null

    echo "正在删除 systemd 服务..."
    rm -f "/etc/systemd/system/${MODEL_VIEWER_SERVICE}.service"
    systemctl daemon-reload 2>/dev/null

    echo "正在删除文件..."
    rm -rf "$MODEL_VIEWER_INSTALL_DIR"

    echo ""
    echo -e "${gl_lv}✅ 模型列表查看器卸载完成${gl_bai}"
...
manage_model_viewer() {
    while true; do
        clear
        echo -e "${gl_kjlan}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${gl_bai}"
        echo -e "${gl_kjlan}  御三家模型列表 (Claude / GPT / Gemini)${gl_bai}"
        echo -e "${gl_kjlan}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${gl_bai}"
        echo ""
        echo -e "${gl_hui}实时查询三大 AI 提供商的官方模型列表${gl_bai}"
        echo ""

        local status
        status=$(model_viewer_check_status)
